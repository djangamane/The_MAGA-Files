    ...
    for (const item of items) {
      ...
      if (statSync(itemPath).isDirectory()) {
        if (item === 'node_modules') {
          for (const module of modulesToRemove) {
            const nestedModulePath = join(itemPath, module);
            if (existsSync(nestedModulePath)) {
              try {
                rmSync(nestedModulePath, { recursive: true });
                console.log(`Removed nested ${module} from ${itemPath}`);
                removedCount++;
              } catch (e) {
                console.log(`Failed to remove nested ${module} from ${itemPath}: ${e.message}`);
              }
            }
          }
        } else {
          // Recursively check subdirectories
          checkNestedModules(itemPath);
        }
      }
    }
  };
  
  checkNestedModules('node_modules');
} catch (e) {
  console.log(`Error checking nested node_modules: ${e.message}`);
}

console.log(`Removed ${removedCount} problematic modules`);

// Patch all rollup native.js files to prevent loading problematic modules
try {
  const patchRollupNativeFiles = (dir) => {
    const items = readdirSync(dir);
    for (const item of items) {
      const itemPath = join(dir, item);
      if (statSync(itemPath).isDirectory()) {
        // Check if this is a rollup directory
        if (item === 'rollup' && existsSync(join(itemPath, 'dist', 'native.js'))) {
          const rollupNativePath = join(itemPath, 'dist', 'native.js');
          try {
            ...
            if (patched) {
              ...
              console.log(`Patched rollup native.js in ${itemPath} to skip native modules`);
            } else {
              console.log(`rollup native.js in ${itemPath} already patched or structure changed`);
            }
          } catch (e) {
            console.log(`Failed to patch rollup native.js in ${itemPath}: ${e.message}`);
          }
        } else {
          // Recursively check subdirectories
          patchRollupNativeFiles(itemPath);
        }
      }
    }
  };
  
  patchRollupNativeFiles('node_modules');
} catch (e) {
  console.log(`Error patching rollup native.js files: ${e.message}`);
}
